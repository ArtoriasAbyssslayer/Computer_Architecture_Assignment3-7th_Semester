...
Παράγραφος 8 Validation ~ [mcpat_pdf](https://www.hpl.hp.com/research/mcpat/McPATAlpha_TechRep.pdf)
"Table 4 compares the published die sizes of the target processors with the McPAT results,  which shows that themodeled area numbers track the published numbers well.  The error is higher for Niagara2 because it has more typesof I/O components than others as mentioned above, which are not modeled in McPAT. It is important to point out thateven if we use empirical models for highly irregular logic, we still see a reasonable match between McPAT results andreported data."
### Πηγές 
